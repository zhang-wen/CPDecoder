...
from utils import exeTime, back_tracking, _filter_reidx, \
        init_beam_sm, part_sort, debug, debug
import numpy as np
import time
import sys
...
class Func(object):
    ...
    def __init__(self, lqc, fs, switchs):
        ...
        self.lqc = lqc
        self.f_init, self.f_nh, self.f_na, self.f_ns, self.f_mo, \
                self.f_pws, self.f_one, self.f_ce, self.f_next, self.f_emb = fs

        self.ifvalid, self.ifbatch, self.ifscore, self.ifnorm, self.ifmv, self.ifwatch_adist, \
            self.merge_way, self.ifapprox_dist, self.ifapprox_att, self.ifadd_lmscore, \
                self.ifsplit = switchs
    ...
    def fn_next(self, y_im1, ctx, s_im1):
        self.lqc[8] += 1
        return self.f_next(*[y_im1, ctx, s_im1]) if self.ifbatch else \
                self.f_next(*[[y_im1], ctx, s_im1])
...
class NBS(Func):
    ...
    def beam_search_trans(self, src_sent):
        ...
        debug('init[{}] nh[{}] na[{}] ns[{}] mo[{}] ws[{}] ps[{}] ce[{}] next[{}]'.format(*self.lqc[0:9]))
    ...
    def beam_search(self, np_src_sent):
        ...
        init_beam_sm(self.beam, cnt=maxlen, init_state=s_init, init_y_emb_im1=y_emb_im1)
        ...
        for i in range(1, maxlen + 1):
            ...
            for j in xrange(len(self.beam[i - 1])):  # size of last beam
                ...
                cands += [(accum_i[idx], si, wid, self.fn_emb([wid]), j) for idx, wid in enumerate(ranks_idx_flat)]
    ...
    def beam_search_comb(self, np_src_sent):
        ...
        detail = False
        y_emb_im1 = self.fn_emb([-1])
        init_beam_sm(self.beam, cnt=maxlen, init_state=s_init[0], init_y_emb_im1=y_emb_im1)
        for i in range(1, maxlen + 1):
            # beam search here
            if (i - 1) % 10 == 0:
                debug(str(i - 1))
            ...
            for b in zip(costs, si[prevb_id], word_indices, prevb_id):
                if b[2] == self.eos_id:
                    ...
                    self.locrt[0] += (b[-1] + 1)
                    self.locrt[1] += 1
                    self.beam[i].append((b[0], b[1], b[2], self.fn_emb([b[2]]), b[3]))
            debug('beam {} ----------------------------'.format(i))
            for b in self.beam[i]:
