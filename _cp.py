...
class WCP(Func):
    ...
    def cube_prune_trans(self, src_sent):
        ...
        init_beam(self.beam, cnt=self.maxlen, init_state=s_im1, detail=False, cp=True)
    ...
    def create_cube(self, bidx, eq_classes):
        ...
        for whichsubcub, leq_class in eq_classes.iteritems():   # sub cube
            ...
            if self.lm is not None and bidx >= 4:
                ...
                _next_krank_ids = part_sort(-_avg_scores_i_flat, self.k - cnt_transed)
                ...
                self.approx_items.append((_y_emb_im1, _avg_hi, _avg_ai, _avg_si, _avg_moi,
    ...
    def Push_heap(self, heap, bidx, citem):
        ...
        if self.lm is not None and bidx >= 4:
            ...
            if self.ifapprox_dist == 1:
                ...
                if self.ifapprox_att == 1:
                    ...
                    if self.cube_lines_mergeout[which][iexp] is not None:
                        ...
                        if self.approx_items[which] is not None:
                            (_y_emb_im1, _avg_hi, _avg_ai) = self.approx_items[which]
                        else:
                            _y_emb_im1, _avg_hi = self.fn_nh(y_im1, _avg_sim1)
                            _avg_pi, _avg_ai = self.fn_na(self.context, self.uh, _avg_hi)
                            self.approx_items[which] = (_y_emb_im1, _avg_hi, _avg_ai)
            ...
            (_y_emb_im1, _avg_hi, _avg_ai, _avg_si, _avg_moi, \
             _avg_scores_i, _next_krank_wids) = self.approx_items[which]

            _avg_ces_i = self.fn_ce(_avg_scores_i)
            _f_avg_ce_i = _avg_ces_i.flatten()[yi]
            _ce_i = score_im1 + _f_avg_ce_i
            ...
            if rsz == 1 or self.ifapprox_dist == 1:
                ...
                if self.ifapprox_att == 1:
                    ...
                    if self.cube_lines_mergeout[which][iexp] is not None:
                        ...
                        _avg_moi = self.fn_mo(y_emb_im1, _avg_ai, true_si)
                        self.cube_lines_mergeout[which][iexp] = true_si, _avg_moi
                    ...
                    if self.ifscore:
                        ...
                        sci = self.fn_pws(_avg_moi, self.ptv)
                        cei = self.fn_ce(sci).flatten()[yi]
                        true_sci = score_im1 + cei
                        debug('P| {}={}+{}'.format(format(true_sci, '0.3f'),
                                                   format(score_im1, '0.3f'),
                                                   format(cei, '0.3f')))
                else:
                    debug('real distribution ...')
                    if self.cube_lines_mergeout[which][iexp] is not None:
                        true_si, cei = self.cube_lines_mergeout[which][iexp]
                    else:
                        y_emb_im1, hi = self.fn_nh(y_im1, s_im1)
                        ai = self.fn_na(self.context, self.uh, hi)[1]
                        true_si = self.fn_ns(hi, ai)
                        ...
                        sci = self.fn_pws(moi, self.ptv)
                        cei = self.fn_ce(sci)
                        self.cube_lines_mergeout[which][iexp] = true_si, cei
        ...
        heapq.heappush(heap, (score_im1 + _ce_i, true_sci, next(self.cnt), \
                              true_si, yi, bp, iexp, jexp, which))
        debug('###########################################################################')
    ...
    def cube_prune(self, bidx, cube):
        ...
        cnt_transed = len(self.translations)
        while len(extheap) > 0:
            _ce_i, true_sci, _, true_si, yi, bp, iexp, jexp, which = heapq.heappop(extheap)
            prev_pop_i, prev_pop_j = ijs_pop[which]
            if iexp > prev_pop_i:
                assert(jexp <= prev_pop_j)
                debug('subcube {}: {}-{}-{}-{}, down shift pop ... {} {}'.format(
                    which, prev_pop_i, prev_pop_j, iexp, jexp, true_sci, yi))
            elif jexp > prev_pop_j:
                assert(iexp <= prev_pop_i)
                debug('subcube {}: {}-{}-{}-{}, right shift pop ... {} {}'.format(
                    which, prev_pop_i, prev_pop_j, iexp, jexp, true_sci, yi))
            ijs_pop[which] = (iexp, jexp)
            if yi == self.eos_id:
